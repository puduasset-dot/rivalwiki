...
export function CrosshairGeneratorClientZh() {
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <label className="block space-y-1">
            <span className="text-xs text-neutral-500">大小: {size}</span>
            <input type="range" min={2} max={20} value={size} onChange={(e) => setSize(Number(e.target.value))} className="w-full" />
          </label>

          <label className="block space-y-1">
            <span className="text-xs text-neutral-500">间隙: {gap}</span>
        ...
        <div className="space-y-4">
          <label className="block space-y-1">
            <span className="text-xs text-neutral-500">粗细: {thickness}</span>
      ...
      <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-4 space-y-2">
        ...
        <p className="text-xs text-neutral-500">
          RIVALS 没有准星导入码——复制这些数值,然后在游戏内的准星设置里手动填入对应数值。
        </p>
      </div>
    </div>
  );
