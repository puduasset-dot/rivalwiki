import type { Metadata } from "next";
import { codes, CODES_SOURCE } from "@/data/codes";
import { howToRedeemZh, codeStatusZh } from "@/data/i18n/zh/codes";
import { SourceNote } from "@/components/SourceNote";
import { PageBanner } from "@/components/PageBanner";

export const metadata: Metadata = {
  title: "RIVALS 兑换码",
  description: "当前有效的 RIVALS(Roblox)兑换码及对应奖励。",
};

export default function CodesPageZh() {
  return (
    <div className="space-y-6">
      <PageBanner src="/media/rivals-logo-screen.jpg" alt="RIVALS title screen" />
      <div className="space-y-2">
        <h1 className="text-2xl font-bold text-white">RIVALS 兑换码</h1>
        <SourceNote label={CODES_SOURCE.label} lastChecked={CODES_SOURCE.lastChecked} />
        <p className="text-sm text-neutral-400">
          兑换码失效很快,不同信息源对哪些还有效的说法也常常不一致——想要的码建议立刻去试。
        </p>
      </div>

      <div className="overflow-hidden rounded-lg border border-neutral-800">
        <table className="w-full text-sm">
          <thead className="bg-neutral-900 text-neutral-400 text-left">
            <tr>
              <th className="px-4 py-2 font-medium">兑换码</th>
              <th className="px-4 py-2 font-medium">奖励</th>
              <th className="px-4 py-2 font-medium">状态</th>
            </tr>
          </thead>
          <tbody>
            {codes.map((c) => (
              <tr key={c.code} className="border-t border-neutral-800">
                <td className="px-4 py-2 font-mono text-amber-400">{c.code}</td>
                <td className="px-4 py-2 text-neutral-200">{c.reward}</td>
                <td className="px-4 py-2">
                  <span className={c.status === "active" ? "text-green-400" : "text-neutral-500"}>
                    {codeStatusZh[c.status]}
                  </span>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>

      <div className="space-y-2">
        <h2 className="font-semibold text-white">兑换方法</h2>
        <ol className="list-decimal list-inside text-sm text-neutral-400 space-y-1">
          {howToRedeemZh.map((step, i) => (
            <li key={i}>{step}</li>
          ))}
        </ol>
      </div>
    </div>
  );
}
