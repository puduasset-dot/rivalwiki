...
const FIELDS: { key: keyof Weapon; label: string }[] = [
  ...
  { key: "fireRate", label: "連射速度" },
  { key: "ammo", label: "弾数" },
  { key: "reload", label: "リロード" },
];
...
export function WeaponComparisonClientJa() {
  ...
  const nameA = weaponA ? weaponNamesJa[weaponA.slug] ?? weaponA.name : "";
  const nameB = weaponB ? weaponNamesJa[weaponB.slug] ?? weaponB.name : "";

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 items-end">
        <WeaponSelect label="武器 A" value={slugA} onChange={setSlugA} />
        <span className="text-neutral-500 pb-2 hidden sm:block">対</span>
      ...
      {weaponA && weaponB ? (
        <>
          <div className="flex items-center justify-center gap-6">
            ...
            <span className="text-neutral-500 text-sm">対</span>
            <div className="flex flex-col items-center gap-2">
              <div className="w-12 h-12 rounded-lg bg-neutral-800 flex items-center justify-center overflow-hidden">
