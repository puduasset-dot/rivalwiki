...
import { buildWeaponPairs, getWeaponPair, buildVerdictJa } from "@/data/weapon-compare";
...
export async function generateMetadata(props: PageProps<"/ja/weapons/compare/[pair]">): Promise<Metadata> {
  const { pair } = await props.params;
  const found = getWeaponPair(pair);
  if (!found) return {};
  const nameA = weaponNamesJa[found.a.slug] ?? found.a.name;
  const nameB = weaponNamesJa[found.b.slug] ?? found.b.name;
  return {
    title: `${nameA} 対 ${nameB} — RIVALS 武器比較`,
    description: `${nameA} と ${nameB} の比較：ダメージ、連射速度、弾数、リロード、コストを並べて確認。`,
  };
}

const MAX_RPS = 1 / 0.07;

export default async function WeaponComparePageJa(props: PageProps<"/ja/weapons/compare/[pair]">) {
  const { pair } = await props.params;
  const found = getWeaponPair(pair);
  if (!found) notFound();
  const { a, b } = found;
  const nameA = weaponNamesJa[a.slug] ?? a.name;
  const nameB = weaponNamesJa[b.slug] ?? b.name;

  const verdict = buildVerdictJa(a, b, nameA, nameB);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-center gap-6">
        {[{ w: a, name: nameA }, { w: b, name: nameB }].map(({ w, name }, i) => (
          <div key={w.slug} className="flex items-center gap-3">
            <Link href={`/ja/weapons/${w.slug}`} className="flex flex-col items-center gap-2 group">
      ...
      <div className="overflow-hidden rounded-lg border border-neutral-800">
        <table className="w-full text-sm">
          <thead className="bg-neutral-900 text-left">
            <tr>
              <th className="px-4 py-2 font-medium text-neutral-400">項目</th>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {[{ w: a, name: nameA }, { w: b, name: nameB }].map(({ w, name }) => (
          <div key={w.slug} className="space-y-1.5">
            ...
            {weaponOverviewsJa[w.slug] ? (
              <p className="text-sm text-neutral-400 leading-relaxed">{weaponOverviewsJa[w.slug]}</p>
            ) : (
      ...
      <Link href="/ja/tools/weapon-comparison" className="inline-block text-sm text-amber-400 hover:underline">
        {t.compareAnother}
      </Link>
    </div>
  );
