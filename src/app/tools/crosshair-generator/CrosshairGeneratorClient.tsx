...
export function CrosshairGeneratorClient() {
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <label className="block space-y-1">
            <span className="text-xs text-neutral-500">Size: {size}</span>
            <input type="range" min={2} max={20} value={size} onChange={(e) => setSize(Number(e.target.value))} className="w-full" />
          </label>

          <label className="block space-y-1">
            <span className="text-xs text-neutral-500">Gap: {gap}</span>
        ...
        <div className="space-y-4">
          <label className="block space-y-1">
            <span className="text-xs text-neutral-500">Thickness: {thickness}</span>
            <input type="range" min={1} max={6} value={thickness} onChange={(e) => setThickness(Number(e.target.value))} className="w-full" />
          </label>

          <label className="block space-y-1">
            <span className="text-xs text-neutral-500">Opacity: {opacity}%</span>
      ...
      <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-4 space-y-2">
        ...
        <p className="text-xs text-neutral-500">
          RIVALS doesn&apos;t have a crosshair import code — copy these values
          and match them manually in the in-game crosshair settings.
        </p>
      </div>
    </div>
  );
