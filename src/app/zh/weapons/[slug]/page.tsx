...
export async function generateMetadata(props: PageProps<"/zh/weapons/[slug]">): Promise<Metadata> {
  const { slug } = await props.params;
  const weapon = getWeapon(slug);
  if (!weapon) return {};
  const name = weaponNamesZh[slug] ?? weapon.name;
  const overview = weaponOverviewsZh[slug];
  return {
    title: `${name} — RIVALS 武器数据`,
    description: overview ? overview.slice(0, 100) : `${name} 数据：伤害 ${weapon.damage}，射速 ${weapon.fireRate}。`,
  };
}

const MAX_RPS = 1 / 0.07;

export default async function WeaponPageZh(props: PageProps<"/zh/weapons/[slug]">) {
  const { slug } = await props.params;
  const weapon = getWeapon(slug);
  if (!weapon) notFound();
  const name = weaponNamesZh[slug] ?? weapon.name;
    ...
    <div className="space-y-6">
      ...
      {(weapon.damageValue || rps) && (
        <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-4 space-y-3">
          {weapon.damageValue && (
            <StatBar label={`${t.damage}（近距离）`} value={weapon.damageValue} max={MAX_DAMAGE_VALUE} displayValue={String(weapon.damageValue)} />
          )}
          {rps && (
            <StatBar label={t.fireRate} value={rps} max={MAX_RPS} displayValue={`${rps.toFixed(1)} 发/秒`} colorClass="bg-blue-400" />
          )}
        </div>
      )}
      ...
      <Link href={`/zh/tools/weapon-comparison?a=${weapon.slug}`} className="inline-block text-sm text-amber-400 hover:underline">
        {t.compareWith(name)}
      </Link>
    </div>
  );
